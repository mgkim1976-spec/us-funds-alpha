...
import sys, importlib.util, json, datetime as dt
...
def main():
    ...
    out = dict(generated=dt.date.today().strftime("%Y-%m-%d"), lookback_days=LOOKBACK,
               ...
               events=[e for e in evs if e["ticker"]][:30])
    json.dump(out, open(ROOT/"dashboard"/"sc13d.json", "w"), ensure_ascii=False, indent=1)
    print(f"sc13d.json: {out['n_total']}건(신규 {out['n_new']}) / 최근 {min(30,len(out['events']))} 표시")
    for e in out["events"][:8]:
        print(f"  {e['date']} {e['activist']:12} {'신규' if e['new'] else '수정'} {str(e['ticker']):6} {str(e['name'])[:24]:26} 이후 {e['ret']}%")
