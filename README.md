...
시그널을 만들고, **공개 데이터만으로 point-in-time·팩터조정** 기준으로 검증한
오픈소스 연구 프로젝트입니다.
...
랭크가중 top30, FF5+Mom α)이 이를 보여줍니다 — *이론이 어느 시그널이 왜 사는지까지 설명*합니다:
...
캠페인 개시가 모니터링 창(1년) 밖이면 — 행동주의 캠페인은 수년씩 가므로 대부분이 그렇습니다 —
**대상 회사 기준으로 그 액티비스트의 과거 개시 13D를 소급 추적**해 머리로 붙입니다(`소급` 표식).
이 표적 매칭으로 개시 포착이 28개 중 6 → **23개**로 늘었습니다(나머지 5개는 개시가 EDGAR
최근 제출 이력 밖이거나 다른 신고 주체로 제출돼 미복구). 소급 개시의 이후수익은 수년 누적
시장조정값이라 최근 공시와 한 열에서 오해를 부르므로 생략합니다.

각 공시에는 ⑴ Item 4(투자 목적)에서 추출한 **목적 태그**(이사회·이사 / 매각·합병 / 저평가·자본환원
/ 운영·분사 / 지배구조), ⑵ **데먼드 레터 첨부 여부**(✉), ⑶ **공시 이후 시장조정 초과수익**(−S&P 500),
⑷ EDGAR 원문 링크가 달립니다. 상단에서 **종목·회사 검색 · 액티비스트 · 목적**으로 필터링하고,
**정렬(개시순/활동순)** 과 **🆕 최근개시 필터**를 토글할 수 있습니다.

알파가 개시일 기준이라 — 발표효과는 빠르게 감쇠 — 각 캠페인의 **개시일(최초 공시일)을 함께 표시**하고
**개시순으로 기본 정렬**합니다(최근 개시가 위로). 매매에 유효한 건 *개시가 최근인* 캠페인이므로,
1년 안에 실제로 개시된 캠페인(소급 추적분 제외)은 파란색으로 강조하고 `🆕 최근개시` 필터로 따로
추릴 수 있습니다(120일 이내 개시는 🆕 표시). 5년 전 개시에 최근 수정만 붙은 캠페인은 매매 신호가
아니라는 점이 한눈에 구분됩니다.
